                    ...
                    <?php if (isset($film) && !empty($film)): ?>
                      <?php foreach ($film as $film): ?>
                        <tr>
                          ...
                          <td> <?= $film['DIRECTOR'] ?></td>
                          <td> <?= $film['ACTOR'] ?></td>
                          <td> <?= $film['CODE_TYPE'] ?></td>
                          <td> <?= $film['PREMIERE'] ?></td>
                          <td> <?= $film['TIME'] ?></td>
                          <td> <?= $film['CODE_LANGUAGE'] ?></td>
                          <td> <?= $film['RATED'] ?></td>
                          <td> <img src="../Images/<?= $film["IMAGES"] ?>" alt="images" style=""></td>
                          <td> <?= $film['DISCRIPTION'] ?></td>
                          <td>
                            
                            <button class="btn edit" onclick="editFilm(<?= $film['CODE_FILM'] ?>)">Sửa</button>
                            <button class="btn del" onclick="deleteFilm(<?= $film['CODE_FILM'] ?>)">Xóa</button>
                          </td>
                        </tr>
                        
                      <?php endforeach; ?>
                      <button class="btn add" onclick="addFilm(<?= $film['CODE_FILM'] ?>)">Thêm</button>
                    <?php else: ?>
                      <tr>
                        <td colspan="10">No films found.</td>
                      </tr>
                    <?php endif; ?>
                    </tr>
                  </table>
...
<script>
  function addFilm(codeFilm) {
    window.location.href = "addFilm.php";
  }
</script>
<script>
  function deleteFilm(CODE_FILM) {
    if (confirm("Bạn có chắc chắn muốn xóa phim này không?")) {
      // Gửi yêu cầu xóa thông qua AJAX
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "delFilm.php", true);
      xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          // Xử lý phản hồi từ máy chủ (nếu cần)
          alert(xhr.responseText); // Hiển thị thông báo
          // Cập nhật giao diện người dùng sau khi phim đã được xóa
          // Ví dụ: reload trang hoặc làm mới danh sách phim
          window.location.reload();
        }
      };
      xhr.send("CODE_FILM=" + CODE_FILM);
...
<script>
  function editFilm(codeFilm) {
    window.location.href = "editFilm.php";
  }
</script>
